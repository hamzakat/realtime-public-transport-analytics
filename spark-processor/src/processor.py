...
from pyspark.sql.functions import (
    col, from_json, window, count, approx_count_distinct, avg, min as spark_min, 
    max as spark_max, current_timestamp, to_timestamp, lit
)
    ...
    def run(self):
        ...
        try:
            ...
            kafka_output_query = output_df \
                ...
                .option("checkpointLocation", self.config.checkpoint_location + "/kafka-output") \
                .trigger(processingTime='10 seconds') \
                .start()
            ...
            influx_query = output_df.writeStream \
                ...
                .option("checkpointLocation", self.config.checkpoint_location + "/influxdb-output") \
                .trigger(processingTime='10 seconds') \
                .start()
